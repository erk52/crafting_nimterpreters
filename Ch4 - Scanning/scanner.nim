...
proc scanToken(s: Scanner) = 
  ...
  case c
    ...
    of '!': s.addToken((if s.match('='): TokenKind.BangEqual else: TokenKind.Bang))
    of '=': s.addToken((if s.match('='): TokenKind.EqualEqual else: TokenKind.Equal))
    of '<': s.addToken((if s.match('='): TokenKind.LessEqual else: TokenKind.Less))
    of '>': s.addToken((if s.match('='): TokenKind.GreaterEqual else: TokenKind.Greater))
    of '/':
      if s.match('/'):
        while s.peek() != '\n' and not s.isAtEnd():
          discard s.advance()
